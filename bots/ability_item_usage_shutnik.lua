...
if string.find(GetBot():GetUnitName(), "hero") and build == "NOT IMPLEMENTED" then
	build = require(GetScriptDirectory() .. "/shutnik/item_build_" .. string.gsub(GetBot():GetUnitName(), "npc_dota_hero_", ""))
end
...
function UseGlyph()
	...
	do
		...
		if  tower ~= nil and tower:GetHealth() > 0 and tower:GetHealth()/tower:GetMaxHealth() < 0.08  
		then
