	...
	then
		...
		then
			...
			do
				...
				if((allyfactor<1.2 or ally:GetMaxHealth()-ally:GetHealth()>=300) and GetUnitToUnitDistance(ally,TargetShrine)<=5000-allyfactor*1500) and ally:IsAlive()
				then
...
function Think()
	...
	then
		...
		then
			...
			do
				...
				then
					if(GetUnitToUnitDistance(ally,npcBot.Shrine)>500 and allyfactor<1.2)
					then
						ready=false
					end
				end
	...
	if(npcBot:GetHealth()/npcBot:GetMaxHealth()+npcBot:GetMana()/npcBot:GetMaxMana()<1.2)
	then
